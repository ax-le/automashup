...
class StableAudioPipeline(DiffusionPipeline):
    ...
    def prepare_latents(
        ...
        if initial_audio_waveforms is not None:
            ...
            if initial_audio_waveforms.ndim == 2:
                # [batch_size, audio_length] → [batch_size, 1, audio_length]
                #[2, 441000]→ [2, 1, 441000]
            ...
            #audio_vae_length est la longueur de l'audio brut qu'on doit donner au vae pour qu'il donne
            #en sortie un audio compressé de longueur sample_size qu'attend le transformer en entrée
            #vae.hpo_length est le facteur de compression du vae : combien d'échantillons d'audio brut
            #donnent un échantillon en sortie du vae
            audio_vae_length = int(self.transformer.config.sample_size) * self.vae.hop_length
            # audio_shape va être utilisé pour mettre initial_waveforms à sa forme et ainsi l'encoder avec le vae
            # on divise batch_size // num_waveforms_per_prompt pour éviter d'encoder un même prompt plusieurs fois
            # après l'encodage on remultipliera batch_size * num_waveforms_per_prompt
            audio_shape = (batch_size // num_waveforms_per_prompt, audio_channels, audio_vae_length)

            # check num_channels
            # rajoute des channels fictifs à la waveform si par exemple elle est en mono et le vae attend du stereo
            ...
            # crop or pad
            # si l'audio est trop long/court, il va être pad/crop pour correspondre à la longueur attendue par le vae
            # on affiche ainsi un message d'avertissement
            ...
            # on répète encoded_audio autant de fois qu'on veut générer pour le prompt
            # (batch_size*num_waveforms_per_prompt,num_channels_vae,audio_vae_length)
            encoded_audio = encoded_audio.repeat((num_waveforms_per_prompt, 1, 1))
            # au départ de la diffusion on est à x0 + e*sigma_max
            latents = encoded_audio + latents
        return latents

    def prepare_latents_2(
        self,
        batch_size,
        num_channels_vae,
        sample_size,
        ...
        #on laisse tomber batch_size car on va donner qu'un seul prompt
        #on a sample_size la taille du latent que prend le transformer en entrée
        # et num_channels_vae la dimension du latent
        shape = (batch_size, num_channels_vae, sample_size)
        if isinstance(generator, list) and len(generator) != batch_size:
            raise ValueError(
                f"You have passed a list of generators of length {len(generator)}, but requested an effective batch"
        ...
        if latents is None:
            # On tire un bruit selon la loi normale qui vérifie les dimensions de shape
            latents = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        else:
            latents = latents.to(device)
        ...
        if initial_audio_waveforms is not None:
            # check dimension
            if initial_audio_waveforms.ndim == 2:
                # agit sur le signal audio en passant de (batch, time) à (batch, channels, time)
                # plusieurs channels possible ça dépend du type de son mono, stereo
                initial_audio_waveforms = initial_audio_waveforms.unsqueeze(1)
            elif initial_audio_waveforms.ndim != 3:
                raise ValueError(
                    f"`initial_audio_waveforms` must be of shape `(batch_size, num_channels, audio_length)` or `(batch_size, audio_length)` but has `{initial_audio_waveforms.ndim}` dimensions"
            ...
            # nombre d'échantillons de l'audio qu'on doit donner au vae
            audio_vae_length = int(self.transformer.config.sample_size) * self.vae.hop_length
            # audio_shape rajoute la dimension channels en plus à audio_vae_length
            audio_shape = (batch_size // num_waveforms_per_prompt, audio_channels, audio_vae_length)
            ...
            # Ici initial_audio_waveforms sert juste à dire que le tensor de zeros de taille audio_shape doit être
            # de même dtype et de même device que lui 
            audio = initial_audio_waveforms.new_zeros(audio_shape)
            # Assigne les valeurs de initial_audio_waveform qui ne dépassent pas audio_vae_length
            audio[:, :, : min(audio_length, audio_vae_length)] = initial_audio_waveforms[:, :, :audio_vae_length]

            # on passe de (B,C,T) -> (B,C_latent,T_latent) 
            # .latent_dist.sample(generator) tire un latent selon la loi du vae
            encoded_audio = self.vae.encode(audio).latent_dist.sample(generator)
            # (B,C_latent,T_latent)  -> (B*num_waveforms_per_prompt,C_latent,T_latent)
            encoded_audio = encoded_audio.repeat((num_waveforms_per_prompt, 1, 1))


            
        return latents, encoded_audio
    ...
    def __call__(
        ...
        # Encode duration
        # Stable audio permet de générer indépendement des segments sur différentes plages de longueurs
        # exemple : chunk 1 : 0s → 10s  → prompt: "intro calm piano" chunk 2 : 10s → 20s → prompt: "drums enter" chunk 3 : 20s → 30s → prompt: "full orchestra"
        # puis on peut concaténer les audio bruts, mais les transitions ne seront pas fluides comme les segments sont générés de manière indépendante
        # On peut alors gérer les transitions en post-processing avec par exemple des fades
        ...
        # (nb_prompts,nb_tokens,dim_embedding) -> (nb_prompts*num_waveforms_per_prompt,nb_tokens,dim_embedding)
        ...
        print(f"num channels vae {num_channels_vae}")
    ...
    @torch.no_grad()
    @replace_example_docstring(EXAMPLE_DOC_STRING)
    #Fonction pour le masking
    def audio_inpaint(
        self,
        prompt: Union[str, List[str]] = None,
        audio_end_in_s: Optional[float] = None,
        audio_start_in_s: Optional[float] = 0.0,
        mask_end_in_s : Optional[float] = None,
        mask_start_in_s: Optional[float] = 0.0,
        num_inference_steps: int = 100,
        guidance_scale: float = 7.0,
        negative_prompt: Optional[Union[str, List[str]]] = None,
        num_waveforms_per_prompt: Optional[int] = 1,
        ...
        """
        # 0. Convert audio input length from seconds to latent length
        # On a de base N échantillons dans le signal sonore
        # Avec la compression du signal par le vae, on obtient un vecteur latent où une coordonée = downsample_ratio échantillons
        #du signal de départ
        downsample_ratio = self.vae.hop_length

        #le transformer prend en entrée des latents de taille maximale sample_size
        # sampling_rate est le nombre d'échantillons par seconde dans le signal de départ (pourquoi le vae a cette information?)
        #on divise donc le nombre d'échantillons max que le transformer peut prendre par le nombre d'échantillons en 1s
        #on obtient max_audio_length_in_s la durée audio maximum que le transformer peut prendre
        max_audio_length_in_s = self.transformer.config.sample_size * downsample_ratio / self.vae.config.sampling_rate
        if audio_end_in_s is None:
            audio_end_in_s = max_audio_length_in_s
        ...
        # Encode duration 
        # Retourne deux embeddings associés au temps de début et au temps de fin
        # Ces embeddings sont donnés au transformer pour qu'il sache où générer
        seconds_start_hidden_states, seconds_end_hidden_states = self.encode_duration(
            audio_start_in_s,
            audio_end_in_s,
            device,
        ...
        # 5. Prepare latent variables
        num_channels_vae = self.transformer.config.in_channels
        latents, encoded_audio = self.prepare_latents_2(
            batch_size * num_waveforms_per_prompt,
            num_channels_vae,
            waveform_length,
            text_audio_duration_embeds.dtype,
        ...
        mask[:, :, mask_start_latent:mask_end_latent] = 0.0
    ...
    def get_score_rec_guidance(self, x, x_hat, y, t_i, degradation):
        ...
        x_hat=self.proj_convex_set(x_hat.detach(),y,degradation)
    ...
    @torch.no_grad()
    @replace_example_docstring(EXAMPLE_DOC_STRING)
    # Fonction pour l'inpainting score-based
    def score_inpaint(
        self,
        prompt: Union[str, List[str]] = None,
        audio_end_in_s: Optional[float] = None,
        ...
        output_type: Optional[str] = "pt",
        xi : Optional[int] = 0,
        do_score_guidance : Optional[bool]= False,
    ):
        r"""
        The call function to the pipeline for generation.
        ...
        self.xi = xi
        self.do_score_guidance = do_score_guidance
        # 0. Convert audio input length from seconds to latent length
        # On a de base N échantillons dans le signal sonore
        # Avec la compression du signal par le vae, on obtient un vecteur latent où une coordonée = downsample_ratio échantillons
        #du signal de départ
        ...
        # mask*latent fonctionne grâce au broadcasting : la dimension 1 est copiée C fois pour le mask
        # ca ne marche que si des dimensions sont différentes et l'une des deux vaut 1
        mask = torch.ones(
            (B, 1, T),
            device=latents.device,
            dtype=latents.dtype
        ...
        self.y = self.degradation(encoded_audio)
    ...
    @torch.no_grad()
    @replace_example_docstring(EXAMPLE_DOC_STRING)
    #Fonction pour l'inpainting avec cross attn
    def cross_attn_inpaint(
        self,
        prompt: Union[str, List[str]] = None,
        audio_end_in_s: Optional[float] = None,
        audio_start_in_s: Optional[float] = 0.0,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                # noised_encoded_audio = self.scheduler.add_noise(
                #     encoded_audio,
                #     noise,
                #     t.unsqueeze(0) 
                # )
                ...
                #latents = latents * (1 - mask) + noised_encoded_audio * mask
