...
class StableAudioDiTBlock(nn.Module):
    ...
    def forward(
        ...
        # 2.5 Cross-Attention
        norm_hidden_states = self.norm25(hidden_states)

        attn_output = self.attn25(
            norm_hidden_states,
...
class StableAudioDiTModel(ModelMixin, AttentionMixin, ConfigMixin):
    ...
    def __init__(
        ...
        cross_attention_input_dim_audio: int = 64,
    ):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                StableAudioDiTBlock(
                    ...
                    num_kcey_value_attention_heads=num_key_value_attention_heads,
    ...
    def forward(
        ...
        print("edited transformer used")
        cross_attention_hidden_states = self.cross_attention_proj(encoder_hidden_states)
        cross_attention_audio_states = self.cross_attention_proj_audio(audio_hidden_states)
